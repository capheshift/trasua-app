...
var openFB = (function () {
    ...
    function login(callback, options) {
        ...
        requestURL = FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL + '&response_type=token&scope=' + scope;
        loginWindow = window.open(requestURL, '_blank', 'location=no');
    ...
    function dialog(obj) {
        ...
        params['redirect_uri'] = 'http://localhost:4200/';

        url = 'https://www.facebook.com' + obj.path + '?' + toQueryString(params);
        console.log('shareDialog', url);
        shareDialog = window.open(url, '_blank', 'location=no');

        var dialogCallback = function(event){
            var url = event.url;
            if (url.indexOf(params['redirect_uri']) >= 0){
                shareDialog.close();
                obj.success && obj.success({status: 'shared'});
            }
        }
        var dialogExit = function(){
            obj.success && obj.success({status: 'cancelled'});
        }
        if (runningInCordova) {
            shareDialog.addEventListener('loadstart', dialogCallback);
            shareDialog.addEventListener('exit', dialogExit);
        }
    }
